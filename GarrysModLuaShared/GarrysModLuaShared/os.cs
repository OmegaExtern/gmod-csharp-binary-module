...
namespace GarrysModLuaShared
{
    ...
    static class os
    {
        /// <summary>Returns the approximate CPU time the application run.<para/>This can be very useful for optimizing as you can use it to find out how long it took your function to run.</summary>
        /// <param name="luaState">Pointer to lua_State struct.</param>
        /// <returns>The approximate CPU time the application run.</returns>
        public static double clock(IntPtr luaState)
        {
            lock (SyncRoot)
            {
        ...
        /// <param name="luaState">Pointer to lua_State struct.</param>
        /// <param name="format">The format string.<para/>If this is equal to '*t' then this function will return a table, otherwise it will return a string.<para/>If this starts with an '!', the returned data will use the UTC timezone rather than the local timezone.<para/>See http://www.mkssoftware.com/docs/man3/strftime.3.asp for available format flags.<para/>Not all flags are available on all operating systems and the result of using an invalid flag is undefined.This currently crashes the game on Windows.Most or all flags are available on OS X and Linux but considerably fewer are available on Windows.See http://msdn.microsoft.com/en-us/library/fe06s4ak.aspx for a list of available flags on Windows.</param>
        ...
        public static string date(IntPtr luaState, string format, double time)
        {
            lock (SyncRoot)
            {
                if (format == "*t")
        ...
        public static double difftime(IntPtr luaState, double timeA, double timeB)
        {
            lock (SyncRoot)
            {
                lua_getglobal(luaState, nameof(os));
        ...
        public static double time(IntPtr luaState) // TODO: After implementing tables (this takes a DateData structure table).
        {
            lock (SyncRoot)
            {
                lua_getglobal(luaState, nameof(os));
